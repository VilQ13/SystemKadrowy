...
namespace SystemKadrowy.Web.Controllers
{
    public class WyplatyController : Controller
    {
        ...
        public async Task<IActionResult> Oblicz(int id, int? rok, int? miesiac, decimal premia = 0, decimal potracenie = 0, decimal godziny = 168)
        {
            ...
            DateTime dataRozliczenia = new DateTime(r, m, 1);
            ...
            var aktywnaUmowa = pracownik.Umowy
                .Where(u => u.DataRozpoczecia.Date <= dataRozliczenia.Date) // Zaczęła się wcześniej lub teraz
                .Where(u => u.DataZakonczenia == null || u.DataZakonczenia.Value.Date >= dataRozliczenia.Date) // Nie skończyła się
                .FirstOrDefault();
            ...
            if (aktywnaUmowa == null)
            {
                ...
                return Content($"Pracownik {pracownik.Imie} {pracownik.Nazwisko} nie posiada aktywnej umowy w dniu {dataRozliczenia:yyyy-MM-dd}.");
            }

            // Przekazujemy parametry do kalkulatora!// Przekazujemy godziny do kalkulatora
            WynikWyplaty wynik = _kalkulator.Oblicz(aktywnaUmowa, premia, potracenie, godziny);

            ViewBag.WpisanaPremia = premia;
            ViewBag.WpisanePotracenie = potracenie;
            ViewBag.WpisaneGodziny = godziny; // <-- Przekazujemy z powrotem do widoku
        ...
        public async Task<IActionResult> Zatwierdz(int id, int rok, int miesiac, decimal premia = 0, decimal potracenie = 0, decimal godziny = 168)
        {
            ...
            DateTime dataRozliczenia = new DateTime(rok, miesiac, 1);
            ...
            var aktywnaUmowa = pracownik.Umowy
                .Where(u => u.DataRozpoczecia.Date <= dataRozliczenia.Date)
                .Where(u => u.DataZakonczenia == null || u.DataZakonczenia.Value.Date >= dataRozliczenia.Date)
                .FirstOrDefault();
            ...
            var wynik = _kalkulator.Oblicz(aktywnaUmowa, premia, potracenie);
